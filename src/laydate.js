  ...
  */
  ...
  Class.prototype.render = function(){
    ...
    ,elem = that.elem = lay.elem('div', {
      ...
      ,'class': [
        'layui-laydate'
        ,options.range || options.daytime ? ' layui-laydate-range' : ''
        ,options.isHMS ? '':' layui-laydate—daytime '
    ...
    lay.each(new Array(2), function(i){
      ...
      if (options.daytime){
        switch (i) {
          case 0:
          that.config.type = "date"
            break;
          case 1:
          that.config.type = "time"
            break;
        }
      }else{
  ...
  Class.prototype.list = function(type, index){
    ...
    if(type === 'year'){ //年列表
      ...
      if (!options.isHMS) {
          var li = lay.elem('li', {'class': 'layui-laydate-hhmm'}), childUL = ['<p>'+ lang.time[0] +lang.time[1] +'</p><ol>'];
          var select_hhmm = lay.digit(that[startEnd][hms[0]], 2) + ":" + lay.digit(that[startEnd][hms[1]], 2)
          lay.each(new Array(24), function(hh){
            lay.each(new Array(60), function(mm){
              var hhmm = lay.digit(hh, 2) + ":" +  lay.digit(mm, 2)
              childUL.push('<li'+ (select_hhmm === hhmm ? ' class="'+ THIS +'"' : '') +'>'+ hhmm +'</li>');
              // lay.each(new Array(60), function(ss){
              //     childUL.push('<li>'+ lay.digit(hh, 2) + ":" +  lay.digit(mm, 2) + ":" + lay.digit(ss, 2) +'</li>');
              // });
            });
          });
    ...
    if(type === 'year' || type === 'month'){      
      ...
      var span = lay.elem('span', {
        ...
        lay(ul).find('ol').each(function(i){
          ...
          if (options.isHMS){
            ...
            ol.scrollTop = 30*(that[startEnd][hms[0]]*60 + that[startEnd][hms[1]] - 2);
          }
          if(ol.scrollTop <= 0){
      ...
      lay(ul).find('ol').each(function(i){
        ...
        lay(ol).find('li').on('click', function(){
          ...
          if (!options.isHMS){
            var h_m_s = this.innerHTML.split(':')
            lay.each(h_m_s,function(i){
                dateTime[hms[i]] = parseInt(h_m_s[i])
            })
          }
  ...
  Class.prototype.closeList = function(){
    ...
    lay.each(that.elemCont, function(index, item){
      if (options.daytime && index == 1) return;
  ...
  Class.prototype.choose = function(td){
    ...
    } else if(options.type === 'datetime' || options.daytime){
      setDateTime(true);
